...
class Players:
    ...
    def remove_duplicates(self):
        ...
        print(
            "After removing duplicates there are only {} players".format(len(players))
        )
        ...
        print(
            "After removing banned players there are only {} players".format(
                len(players)
            )
        )
    ...
    def print_all(self):
        for player in self.players:
            print(
                "{} {} {} - {}".format(
                    player.first_name, player.last_name, player.team, player.positions
                )
            )
        ...
        for round, played in enumerate(self.played):
            if played:
                ...
                if len(last_three_games) == 3:
                    price_change = (
                        (sum(last_three_games) / 3 * 5140) - self.price[round]
                    ) / 4
