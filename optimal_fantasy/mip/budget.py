...
class MIP:
            ...
            with open(output_file, "w") as f:
                ...
                for round in range(num_rounds):
                    ...
                    for player_id, player in enumerate(model._players):
                        for pos_index, pos in enumerate(model._positions):
                            if pos in model._scoring_positions:
                                if (
                                    model.cbGetSolution(
                                        my_vars["pos_vars"][player_id][round][pos_index]
                                    )
                                    == 1
                                ):
                                    points_in_round = (
                                        points_in_round
                                        + model._players.players[player_id].points[
                                            round
                                        ]
                                    )
                    ...
                    for player_id, player in enumerate(model._players):
                        if (
                            model.cbGetSolution(
                                my_vars["player_vars"][player_id][round]
                            )
                            == 1
                        ):
                    ...
                    if round > 0:
                        for player_id, player in enumerate(model._players):
                            if (
                                model.cbGetSolution(
                                    my_vars["trade_in_vars"][player_id][round - 1]
                                )
                                == 1
                                and model.cbGetSolution(
                                    my_vars["trade_out_vars"][player_id][round - 1]
                                )
                                == 0
                            ):
                                f.write(
                                ...
                                change_in_budget = (
                                    change_in_budget - player.price[round]
                                )
                        for player_id, player in enumerate(model._players):
                            if (
                                model.cbGetSolution(
                    ...
                    for player_id, player in enumerate(model._players):
                        if (
                            model.cbGetSolution(
                                my_vars["captain_vars"][player_id][round]
                            )
                            == 1
                        ):
                            f.write(
                    ...
                    for pos_index, pos in enumerate(model._positions):
                        ...
                        for player_id, player in enumerate(model._players):
                            if (
                                ...
                                if (
                                    model.cbGetSolution(
                                        my_vars["scoring_vars"][player_id][round]
                                    )
                                    == 1
                                ):
                                    points_in_round += player.points[round]

                        players_in_round.sort(
                            key=lambda x: x.points[round], reverse=True
                        )
    ...
    def __initial_budget_lazy(model, where):
        if where == GRB.Callback.MIPSOL:
            ...
            if obj > model._min_score:
                remaining_budgets = [
                    int(model.cbGetSolution(my_vars["budget_vars"][round]))
                    for round in range(model._num_rounds)
                ]
                ...
                model.cbLazy(my_vars["starting_budget"] <= new_budget)
    ...
    def read_starting_solution(self):
        with open("SC-Sub1.txt", "rU") as f_in:
            ...
            for round in range(0, self.num_rounds):
                ...
                line = [
                    (row, row_index)
                    for row_index, row in enumerate(round_data)
                    if "Captain" in row
                ]
            ...
            for round in range(self.num_rounds):
                ...
                player_scoring_vars.append(
                    self.model.addVar(
                        ...
                        obj=objective,
                        name="Scoring-{}-{}-{}".format(
                            player.first_name, player.last_name, round
                        ),
                    )
                )
                    ...
                    player_round_pos_vars.append(
                        self.model.addVar(
                            ...
                            name="PlayerPos-{}-{}-{}-{}".format(
                                player.first_name, player.last_name, round, pos
                            ),
                        )
                    )
    ...
    def create_budget_variables(self):
        ...
        for round in range(self.num_rounds):
            budget_vars.append(
                self.model.addVar(
                    vtype=GRB.CONTINUOUS, obj=0, name="Budget-{}".format(round)
                )
            )
        return budget_vars
            ...
            for round in range(self.num_rounds):
                team_vars.append(
                    self.model.addVar(
                        ...
                        obj=0,
                        name="InTeam-{}-{}-{}".format(
                            player.first_name, player.last_name, round + 1
                        ),
                    )
                )
    ...
    def create_trade_out_variables(self):
        ...
        for player in self.players:
            trade_out_vars = []
            for round in range(1, self.num_rounds):
                trade_out_vars.append(
                    self.model.addVar(
                        vtype=GRB.BINARY,
                        obj=0,
                        name="TradeOut-{}-{}-{}".format(
                            player.first_name, player.last_name, round
                        ),
                    )
                )
        ...
        self.model.addConstr(
            sum(
                ...
                * (
                ...
                for player_id, player in enumerate(self.players.players)
                for round in range(self.num_rounds)
            )
            >= self.min_score + 1
        )
        ...
        for player_id, player in enumerate(self.players):
            for round in range(self.num_rounds):
                self.model.addConstr(
                    self.scoring_vars[player_id][round]
                    - sum(
                        self.pos_vars[player_id][round][pos_index]
    ...
    def add_one_captain_per_round_constraint(self):
        ...
        for round in range(self.num_rounds):
            self.model.addConstr(
                sum(
                    self.captain_vars[player][round]
    ...
    def add_season_trade_constraints(self):
        ...
        self.model.addConstr(
            sum(
                self.trade_out_vars[player][round]
                for player in range(self.num_players)
    ...
    def add_position_constraints(self):
        ...
        for round in range(self.num_rounds):
            for pos_iter, pos in enumerate(self.positions):
                self.model.addConstr(
                    sum(
                        self.pos_vars[player][round][pos_iter]
                        for player in range(self.num_players)
                    )
    ...
    def add_player_position_constraints(self):
        ...
        for round in range(self.num_rounds):
            for player_id, player in enumerate(self.players):
                self.model.addConstr(
                    sum(
                        self.pos_vars[player_id][round][pos_iter]
                        for pos_iter, pos in enumerate(self.positions)
                    )
                    <= 1,
                    "SinglePos-{}-{}-{}".format(
    ...
    def add_consistency_constraints(self):
        ...
        for player_id, player in enumerate(self.players):
            for round in range(self.num_rounds - 1):
                self.model.addConstr(
                    self.player_vars[player_id][round + 1]
    ...
    def add_budget_consistency_constraints(self):
        ...
        for round in range(self.num_rounds - 1):
            ...
            self.model.addConstr(
                self.budget_vars[round + 1]
                - self.budget_vars[round]
                + sum(
                    self.players.players[player].price[round + 1]
                    * self.trade_in_vars[player][round]
                    for player in range(self.num_players)
                )
                + sum(
                    -self.players.players[player].price[round + 1]
    ...
    def add_non_negative_budget_constraints(self):
        ...
        for round in range(self.num_rounds):
            self.model.addConstr(
                self.budget_vars[round] >= 0, "Budget-NonNeg-{}".format(round)
            )
        ...
        with open(output_file, "a") as f:
            ...
            for round in range(self.num_rounds):
                ...
                if round > 0:
                    for player_id, player in enumerate(self.players):
                        if (
                            self.trade_in_vars[player_id][round - 1].x == 1
                            and self.trade_out_vars[player_id][round - 1].x == 0
                        ):
                            f.write(
                                "Trade in {} {} - ${}\n".format(
                                    player.first_name,
                    ...
                    for player_id, player in enumerate(self.players):
                        if (
                            self.trade_out_vars[player_id][round - 1].x == 1
                            and self.trade_in_vars[player_id][round - 1].x == 0
                        ):
                            f.write(
                                "Trade out {} {} - ${}\n".format(
                                    player.first_name,
