...
def read_player_details(player_id):
    ...
    link = "https://api-news-afl.virtualsports.com.au/v1/players/{}?embed=notes,odds,player_stats,positions,trades".format(
        player_id)

    page = requests.get(link)
    data = page.json()
    player_string = "{} {} ({})".format(data[FIRST_NAME], data[LAST_NAME], data['team']['abbrev'])
...
def read_player_projections(player_id, auth):
    ...
    url = "https://api-news-afl.virtualsports.com.au/v1/players/{}/projected_stats".format(player_id)
